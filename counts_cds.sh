...
find /home/baezvg/ncbi_dataset/data/ -name "GCA*.fna" | while read fna_file; do
    ...
    if [[ -f "$gff_file" ]]; then
        ...
        echo "$base_name: $num_cds CDS" >> prokka_output/counts_cds.txt
    else
        echo "$base_name: No GFF file found" >> prokka_output/counts_cds.txt
    fi
done

echo "Annotation complete. CDS counts saved to prokka_output/counts_cds.txt."
